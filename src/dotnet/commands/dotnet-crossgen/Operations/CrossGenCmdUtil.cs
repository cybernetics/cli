...
namespace Microsoft.DotNet.Tools.CrossGen.Operations
{
    public class CrossGenCmdUtil
    {
        ...
        public bool ShouldExclude(string assemblyLocation)
        {
            var fileName = Path.GetFileName(assemblyLocation);
            // skipping CrossGen for excluded and already crossgen'd libraries
            var hasMetadata = PEUtils.HasMetadata(assemblyLocation);
            var exclude = s_excludedLibraries.Any(lib => string.Equals(lib, fileName, StringComparison.OrdinalIgnoreCase)) || !hasMetadata;
            if (exclude)
            {
                Reporter.Verbose.WriteLine($"Excluding {assemblyLocation} for crossgen, has metadata {hasMetadata}");
            }
            return exclude;
        }
        ...
        public ICollection<string> CrossGenAssembly(string appPath, string assemblyLocation, IList<string> platformAssembliesPaths, string outputDirectory, bool generateSymbols)
        {
            ...
            var crossgenArgs = string.Join(" ", GetArgs(appPath, module, platformAssembliesPaths, outputDirectory, false));
            ...
            {
                throw new CrossGenException($"Crossgen module {module} failed.");
            }
            ...
            if (generateSymbols)
            {
                var pdbArgs = string.Join(" ", GetArgs(appPath, module, platformAssembliesPaths, outputDirectory, true));
                ...
                {
                    throw new CrossGenException("Symbol generation for module {module} failed.");
                }
        ...
        private List<string> GetArgs(string appPath, string module, IList<string> platformAssembliesPaths, string outputDirectory, bool isSymbolGeneration)
        {
